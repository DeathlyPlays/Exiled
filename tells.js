...
exports.sendTell = function (userid, user) {
	...
	for (let i = 0; i < tellsToSend.length; i++) {
		...
		buffer += '<div class="chat"><font color="gray">[' + timeStr + ' ago]</font> ' + Exiled.nameColor(toId(tellsToSend[i].sender), true) + tellsToSend[i].sender + ':</font></b> ' + Chat.escapeHTML(tellsToSend[i].msg.replace(/\|/g, '&#124;')) + '</div>';
	}
	user.send(buffer);
