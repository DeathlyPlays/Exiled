	...
	onEnd(reset, endedBy) {
		...
		if (!reset) {
			...
			this.announce(
				...
				`${this.completed.slice(0, sliceIndex).map((p, i) => `${formatOrder(i + 1)} place: <em>${Server.nameColor(p.name)}</em> <span style="color: lightgreen;">[${p.time}]</span>.<br />`).join("")}${this.completed.length > sliceIndex ? `Consolation Prize: ${this.completed.slice(sliceIndex).map(e => `<em>${Chat.escapeHTML(e.name)}</em> <span style="color: lightgreen;">[${e.time}]</span>`).join(', ')}<br />` : ''}<br />` +
...
let commands = {
	...
	status: function (target, room, user) {
		...
		if (game.hosts.some(h => h.userid === user.userid) || game.staffHostId === user.userid) {
			...
			str += Chat.html`<tr><td>Completed</td><td>${completed.length ? completed.map(pl => Server.nameColor(pl.name)).join(", ") : 'None'}`;
