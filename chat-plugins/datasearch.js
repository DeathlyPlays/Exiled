...
function runDexsearch(target, cmd, canAll, message) {
	...
	for (let i = 0; i < andGroups.length; i++) {
		...
		if (parameters.length > 3) return {
			reply: "No more than 3 alternatives for each parameter may be used."
		};
		for (let j = 0; j < parameters.length; j++) {
			...
			if (target in allTiers) {
				...
				if (target === "CAP") {
					if (parameters.length > 1) return {
						reply: "The parameter 'CAP' cannot have alternative parameters"
					};
					if (capSearch === isNotSearch) return {
						reply: "A search cannot both include and exclude 'CAP'."
					};
			...
			if (target === 'all') {
				if (!canAll) return {
					reply: "A search with the parameter 'all' cannot be broadcast."
				};
				if (parameters.length > 1) return {
					reply: "The parameter 'all' cannot have alternative parameters"
				};
				showAll = true;
				orGroup.skip = true;
				break;
			...
			if (target === 'megas' || target === 'mega') {
				if (megaSearch === isNotSearch) return {
					reply: "A search cannot include and exclude 'mega'."
				};
				if (parameters.length > 1) return {
					reply: "The parameter 'mega' cannot have alternative parameters"
				};
			...
			if (target === 'recovery') {
				if (parameters.length > 1) return {
					reply: "The parameter 'recovery' cannot have alternative parameters"
				};
			...
			if (target === 'priority') {
				if (parameters.length > 1) return {
					reply: "The parameter 'priority' cannot have alternative parameters"
				};
			...
			if (target.substr(0, 8) === 'resists ') {
				...
				if (targetResist in Tools.data.TypeChart) {
					...
					return {
						reply: "'" + targetResist + "' is not a recognized type."
					};
				}
			}
			...
			if (typeIndex >= 0) {
				...
				if (target in Tools.data.TypeChart) {
					...
					return {
						reply: "'" + target + "' is not a recognized type."
					};
				}
			}
			...
			if (inequality >= 0) {
				if (isNotSearch) return {
					reply: "You cannot use the negation symbol '!' in stat ranges."
				};
				...
				if (!isNaN(targetParts[0])) {
					...
					return {
						reply: "No value given to compare with '" + escapeHTML(target) + "'."
					};
				}
				switch (toId(stat)) {
				case 'attack':
				...
				if (!(stat in allStats)) return {
					reply: "'" + escapeHTML(target) + "' did not contain a valid stat."
				};
				if (!orGroup.stats[stat]) orGroup.stats[stat] = {};
				if (orGroup.stats[stat][direction]) return {
					reply: "Invalid stat range for " + stat + "."
				};
				orGroup.stats[stat][direction] = num;
				continue;
			}
			return {
				reply: "'" + escapeHTML(target) + "' could not be found in any of the search categories."
			};
		}
		searches.push(orGroup);
	}

	if (showAll && searches.length === 0 && megaSearch === null) return {
...
function runMovesearch(target, cmd, canAll, message) {
	...
	for (let i = 0; i < targets.length; i++) {
		...
		if (typeIndex >= 0) {
			target = target.charAt(0).toUpperCase() + target.substring(1, typeIndex);
			if (!(target in Tools.data.TypeChart)) return {
				reply: "Type '" + escapeHTML(target) + "' not found."
			};
			if (!searches['type']) searches['type'] = {};
			if ((searches['type'][target] && isNotSearch) || (searches['type'][target] === false && !isNotSearch)) return {
				reply: 'A search cannot both exclude and include a type.'
			};
		...
		if (target in allCategories) {
			...
			if ((searches['category'][target] && isNotSearch) || (searches['category'][target] === false && !isNotSearch)) return {
				reply: 'A search cannot both exclude and include a category.'
			};
		...
		if (target in allContestTypes) {
			...
			if ((searches['contestType'][target] && isNotSearch) || (searches['contestType'][target] === false && !isNotSearch)) return {
				reply: 'A search cannot both exclude and include a contest condition.'
			};
		...
		if (target in allFlags) {
			...
			if ((searches['flags'][target] && isNotSearch) || (searches['flags'][target] === false && !isNotSearch)) return {
				reply: 'A search cannot both exclude and include \'' + target + '\'.'
			};
		...
		if (target === 'recovery') {
			if (!searches['recovery']) {
				...
				return {
					reply: 'A search cannot both exclude and include recovery moves.'
				};
			}
			continue;
		}
		...
		if (template.exists) {
			if (Object.keys(lsetData).length) return {
				reply: "A search can only include one Pokémon learnset."
			};
			if (!template.learnset) template = Tools.getTemplate(template.baseSpecies);
		...
		if (inequality >= 0) {
			if (isNotSearch) return {
				reply: "You cannot use the negation symbol '!' in quality ranges."
			};
			inequality = target.charAt(inequality);
			let targetParts = target.replace(/\s/g, '').split(inequality);
			...
			if (!(prop in allProperties)) return {
				reply: "'" + escapeHTML(target) + "' did not contain a valid property."
			};
			if (!searches['property']) searches['property'] = {};
			if (direction === 'equal') {
				if (searches['property'][prop]) return {
					reply: "Invalid property range for " + prop + "."
				};
		...
		if (target.substr(0, 8) === 'priority') {
			...
			if (target === "+") {
				...
				return {
					reply: "Priority type '" + target + "' not recognized."
				};
			}
			if (!searches['property']) searches['property'] = {};
			if (searches['property']['priority']) {
				return {
					reply: "Priority cannot be set with both shorthand and inequality range."
				};
			}
			else {
		...
		if (target.substr(0, 7) === 'boosts ') {
			...
			if (!(target in allBoosts)) return {
				reply: "'" + escapeHTML(target.substr(7)) + "' is not a recognized stat."
			};
			if (!searches['boost']) searches['boost'] = {};
			if ((searches['boost'][target] && isNotSearch) || (searches['boost'][target] === false && !isNotSearch)) return {
				reply: 'A search cannot both exclude and include a stat boost.'
			};
		...
		if (target in allStatus) {
			...
			if ((searches['status'][target] && isNotSearch) || (searches['status'][target] === false && !isNotSearch)) return {
				reply: 'A search cannot both exclude and include a status.'
			};
		...
		if (target in allVolatileStatus) {
			...
			if ((searches['volatileStatus'][target] && isNotSearch) || (searches['volatileStatus'][target] === false && !isNotSearch)) return {
				reply: 'A search cannot both exclude and include a volitile status.'
			};
		...
		return {
			reply: "'" + escapeHTML(oldTarget) + "' could not be found in any of the search categories."
		};
	}
...
function runItemsearch(target, cmd, canAll, message) {
	...
	if (searchedWords.length === 0) return {
		reply: "No distinguishing words were used. Try a more specific search."
	};
	if (searchedWords.includes('fling')) {
		...
		for (let k = 0; k < searchedWords.length; k++) {
			...
			if (wordEff && effect) {
				if (!(wordEff === 'psn' && effect === 'tox')) return {
					reply: "Only specify fling effect once."
				};
			}
			else if (wordEff) {
				effect = wordEff;
				...
				if (Number.isInteger(Number(searchedWords[k]))) {
					if (basePower) return {
						reply: "Only specify a number for base power once."
					};
		...
		if (foundItems.length === 0) return {
			reply: 'No items inflict ' + basePower + 'bp damage when used with Fling.'
		};
	}
		...
		for (let k = 0; k < searchedWords.length; k++) {
			...
			if (searchedWords[k] in Tools.data.TypeChart) {
				if (type) return {
					reply: "Only specify natural gift type once."
				};
		...
		if (foundItems.length === 0) return {
			reply: 'No berries inflict ' + basePower + 'bp damage when used with Natural Gift.'
		};
	}
	else {
		let bestMatched = 0;
...
function runLearn(target, cmd) {
	...
	while (targets.length) {
		...
		if (Tools.getFormat(targetid).exists) {
			if (format.requirePentagon) {
				return {
					error: "'pentagon' can't be used with formats."
				};
			}
	...
	if (!template.exists || template.id === 'missingno') {
		return {
			error: "Pokémon '" + template.id + "' not found."
		};
	}

	if (template.gen > gen) {
		return {
			error: template.name + " didn't exist yet in generation " + gen + "."
		};
	}

	if (targets.length < 2) {
		return {
			error: "You must specify at least one move."
		};
	}
	...
	for (let i = 1, len = targets.length; i < len; i++) {
		...
		if (!move.exists || move.id === 'magikarpsrevenge') {
			return {
				error: "Move '" + move.id + "' not found."
			};
		}
		if (move.gen > gen) {
			return {
				error: move.name + " didn't exist yet in generation " + gen + "."
			};
		}
