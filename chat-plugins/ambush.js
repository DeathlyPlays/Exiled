...
class Ambush {
	constructor(room, seconds) {
		...
		this.room.add('|uhtml|ambush' + this.room.ambushCount + this.round + '|<div style="background-color: #01DFD7 ; border: 12px double #01FF88 ; color: #000000"><h3><img style="transform: scaleX(-1);" src="https://play.pokemonshowdown.com/sprites/xyani/araquanid.gif" height="79" width="110" align="left"><font face="arial" size="4"><center><u><b>A game of Ambush has been started!</b></u></center></font><img src="https://play.pokemonshowdown.com/sprites/xyani/masquerain.gif" height="79" width="110" align="right"></h3><br><br><br><br><br><center>' +
			'The game will begin in <b>' + seconds + '</b> seconds!' +
			'<br><br><button style="border: 4px solid #000 ; background: White ; box-shadow: 0px 1px 1px rgba(255 , 255 , 255 , 0.3) inset ; color: #000000 ; margin: 1px 6px ; padding: 8px 40px" name = "send" value = "/ambush join"><font size="3"><b>Join!</b></font></button><br><br></center></div>'
		);
	...
	updateJoins() {
		let msg = 'ambush' + this.room.ambushCount + this.round + '|<div style="background-color: #01DFD7; border: 12px double #01FF88 ; color: #000000"><center><h3><img style="transform: scaleX(-1);" src="https://play.pokemonshowdown.com/sprites/xyani/araquanid.gif" height="79" width="110" align="left"><font face="arial" size="4"><center><u><b>A game of Ambush has been started!</b></u></center></font><img src="https://play.pokemonshowdown.com/sprites/xyani/masquerain.gif" height="79" width="110" align="right"></h3><br><br><br><br><br><center>' +
			'The game will begin in <b>' + Math.round((this.timeLeft - Date.now()) / 1000) + '</b> seconds' +
			'<br><br><button style="border: 4px solid #000 ; background: White ; box-shadow: 0px 1px 1px rgba(255 , 255 , 255 , 0.3) inset ; color: #000000 ; margin: 1px 6px ; padding: 8px 40px" name = "send" value = "/ambush join"><font size="3"><b>Join!</b></font></button><br><br>';
	...
	leave(user, self) {
		...
		if (!this.round) {
			...
			this.room.add('|html|' + Exiled.nameColor(user.name, true) + ' has left the game!');
		}
	}
	getSurvivors() {
	...
	nextRound() {
		...
		this.round++;
		this.loadGuns();
		let msg = 'ambush' + this.room.ambushCount + this.round + '|<div style="background-color: #01DFD7 ; border: 12px double #01FF88 ; color: #000000"><center><h3><img style="transform: scaleX(-1);" src="https://play.pokemonshowdown.com/sprites/xyani/araquanid.gif" height="79" width="110" align="left"><font face="arial" size="4"><u><b>Round ' + this.round + '</b></u></font><img src="https://play.pokemonshowdown.com/sprites/xyani/masquerain.gif" height="79" width="110" align="right"></h3><br><br><br><br><br><br>' +
	...
	fire(user, target, self) {
		...
		this.madeMove = true;
		if (targetUser === user) {
			this.room.add('|html|' + Exiled.nameColor(user.name, true) + ' shot themself!');
		} else if (!this.players.get(targetUser).rounds) {
			this.room.add('|html|' + Exiled.nameColor(user.name, true) + ' fired at ' + Exiled.nameColor(targetUser.name, true) + ', but ' + Exiled.nameColor(targetUser.name, true) + ' has an active shield!');
			return;
		} else {
			this.room.add('|html|' + Exiled.nameColor(user.name, true) + ' fired at ' + Exiled.nameColor(targetUser.name, true) + '!');
		}
	...
	dq(target, self) {
		...
		this.room.add('|html|' + Exiled.nameColor(targetUser.name, true) + ' has been disqualified from the game.');
		if (this.checkWinner()) this.getWinner();
	}
	checkWinner() {
	...
	getWinner() {
		...
		let msg = '|html|<div class = "infobox"><center>The winner of this game of ambush is ' + Exiled.nameColor(winner, true) + '! Congratulations!</center>';
