...
function validate(me, targetUser, quiet) {
	...
	for (let i in targetUser.movepool) {
		if (!Tools.mod('ssbffa').getMove(targetUser.movepool[i]).exists) {
			valid = false;
...
function buildMenu(userid) {
	...
	let output = '';
	output += '<div class="setchart" style="height: 155px; background-image:url(//play.pokemonshowdown.com/sprites/' + (Tools.getTemplate(toId(Exiled.ssb[userid].species)).gen === 7 ? 'bw' : 'xydex') + '' + (Exiled.ssb[userid].shiny ? '-shiny' : '') + '/' + speciesName + '.png); background-position: -2px -3px; background-repeat: no-repeat;">';
	...
	for (let i = 0; i < statNames.length; i++) {
		...
		if (Tools.getNature(Exiled.ssb[userid].nature).plus === toId(statNames[i])) {
			evBuf += '<small>+</small>';
		} else if (Tools.getNature(Exiled.ssb[userid].nature).minus === toId(statNames[i])) {
...
function abilityMenu(userid) {
	...
	let pokemon = Tools.getTemplate(Exiled.ssb[userid].species);
