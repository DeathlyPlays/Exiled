...
exports.commands = {
	...
	genreq: {
		...
		add: function (target, room, user) {
			...
			if (Users(approvedGenner)) Users(approvedGenner).popup(`|html|You have been approved as a genner by ${Server.nameColor(user.name, true)}.`);
		},
		...
		ban: function (target, room, user) {
			...
			if (Users(targetId)) Users(targetId).popup(`|html|You have been approved as a genner by ${Server.nameColor(user.name, true)}.`);
		},
		...
		update: function (target, room, user) {
			...
			if (status === "genning") {
				if (Users(requesteeId) && Users(requesteeId).connected) Users(requesteeId).send(`|pm|${user.getIdentity()}|${Users(requesteeId).getIdentity()}|/raw ${Server.nameColor(user.name, true)} is currently genning your request.`);
			} else {
				if (Users(requesteeId) && Users(requesteeId).connected) Users(requesteeId).send(`|pm|${user.getIdentity()}|${Users(requesteeId).getIdentity()}|/raw ${Server.nameColor(user.name, true)} has finished your gen request.${requests[requesteeId].reward > 0 ? `<br />You owe ${Server.nameColor(user.name, true)} ${requests[requesteeId].reward} ${requests[requesteeId].reward === 1 ? moneyName : moneyPlural}, failure to do so may result in punishment.` : ``}`);
