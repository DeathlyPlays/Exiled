...
function parseMessage(message, user) {
	...
	case 'c':
		...
		'</small><b><font color="' + Dew.nameColor(name.substr(1)) + '">' + name.substr(1, name.length) + ':</font></b><em>' +
		Dew.parseMessage(lineSplit.slice(3).join('|')) + '</em></span>';
		break;
	case 'c:':
		name = lineSplit[3];
		if (name === '~') break;
		...
		message = '<span class="' + div + '"><small>[' + timestamp + ']</small> ' + '<small>' + name.substr(0, 1) +
		'</small>' + Dew.nameColor(toId(name), true) + '<em>' +
		Dew.parseMessage(lineSplit.slice(4).join('|')) + '</em></span>';
		break;
	case 'uhtml':
