...
<<<<<<< HEAD
			`<p style="text-align:center;font-size:7pt;">Giveaway started by ${Server.nameColor(this.host.name, true)}</p>` +
			`<table style="margin-left:auto;margin-right:auto;"><tr><td style="text-align:center;width:45%">${this.sprite}<p style="font-weight:bold;">Giver: ${Server.nameColor(this.giver)}</p>${Chat.formatText(this.prize, true)}<br />OT: ${Chat.escapeHTML(this.ot)}, TID: ${this.tid}</td>` +
	...
	change(key, value, user) {
		...
		if (force) {
			...
			if (!this.winner) {
				...
				if (this.giver.connected) this.giver.popup(`|raw|${this.winner.name} has won your question giveaway!`);
...
class LotteryGiveaway extends Giveaway {
	...
	drawLottery() {
		...
		if (force) {
			...
			if (this.giver.connected) this.giver.popup(`|raw|The following users have won your lottery giveaway:\n${Server.nameColor(winnerNames)}`);
			Giveaway.updateStats(this.monIds);
		}
		delete this.room.giveaway;
	}
...
class GtsGiveaway {
	...
	generateWindow() {
		...
		return `<p style="text-align:center;font-size:14pt;font-weight:bold;margin-bottom:2px;">There is a GTS giveaway going on!</p>` +
			`<p style="text-align:center;font-size:10pt;margin-top:0px;">Hosted by: ${Server.nameColor(this.giver.name, true, true)} | Left: <b>${this.left}</b></p>` +
