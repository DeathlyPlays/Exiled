...
<<<<<<< HEAD
	generateResults(ended, option, num) {
		...
		let output = `<div class="infobox"><details open><summary style="margin: 2px 0 5px 0">${icon} <strong style="font-size: 11pt">${this.getQuestionMarkup(num)}</strong><psicon pokemon="araquanid"></summary>`;
		output += totalVotes;
...
>>>>>>> 8c668ab0e9ede6de569101e3f57f4bd84bbc0e0d
	...
	tierpoll: function (target, room, user, connection, cmd, message) {
		...
		for (let key in Dex.formats) {
			...
			if (toId(target) !== 'all') {
				let commonMods = ['gen7', 'essb', 'pmd', 'ashspokemon', 'metronome', 'digimon', 'fivemovefrenzy', 'franticfusions', 'holiday', 'mixandmega', 'opmetagame', 'perfection', 'pokebilities', 'slowtown', 'smashingmetagame', 'ssbffa', 'supercell'];
