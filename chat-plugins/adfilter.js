...
Config.chatfilter = function (message, user, room, connection, targetUser) {
	...
	if (!user.can('lock') && (pre_matches >= 1 && final_check === 0 || pre_matches >= 2 && final_check >= 1 || message.match(adRegex))) {
		...
		connection.sendTo(room, '|raw|<strong class="message-throttle-notice">Advertising detected, your message has not been sent and Tsunami global authorities have been notified.' + '<br />Further attempts to advertise in a chat OR PMs will result in being locked.</strong>');
		connection.user.popup("|modal|Advertising detected, your message has not been sent and Tsunami global authorities have been notified.\n" + "Further attempts to advertise in a chat OR in PMs will result in being locked.");
		return false;
	}
	return message;
};
