...
class UNOgame extends Rooms.RoomGame {
	...
	joinGame(user) {
		if (this.state === 'signups' && this.addPlayer(user)) {
			this.sendToRoom(`|html|${Dew.nameColor(user.name, true, true)} has joined the game of UNO.`);
			return true;
		}
		return false;
	}
	...
	getPlayers(showCards) {
		...
		if (!showCards) {
			return playerList.sort().map(id => Dew.nameColor(this.players[id].name, false, true));
		}
		if (this.direction === -1) playerList = playerList.reverse();
		return playerList.map(id => `${(this.currentPlayer && this.currentPlayer === id ? "<strong>" : "")}${Dew.nameColor(this.players[id].name, false, true)} (${this.players[id].hand.length}) ${(this.currentPlayer && this.currentPlayer === id ? "</strong>" : "")}`);
	}
		...
		this.onAwaitUno()
			.then(() => {
				...
				this.timer = setTimeout(() => {
					this.sendToRoom(`${Dew.nameColor(player.name, true, true)} has been automatically disqualified.`);
	...
	onDraw(user) {
		...
		this.sendToRoom(`|html|${Dew.nameColor(user.name, true, true)} has drawn a card.`);
	...
	onPlay(user, cardName) {
		...
		this.sendToRoom(`|raw|${Dew.nameColor(player.name, true, true)} has played a <span style="color: ${textColors[card.color]}">${card.name}</span>.`);
	...
	onRunEffect(value, initialize) {
		...
		case 'Skip':
			this.onNextPlayer();
			this.sendToRoom(`|html|${Dew.nameColor(this.players[this.currentPlayer].name, true, true)}'s turn has been skipped.`);
			break;
		case '+2':
			this.onNextPlayer();
			this.sendToRoom(`|html|${Dew.nameColor(this.players[this.currentPlayer].name, true, true)} has been forced to draw 2 cards.`);
		...
		case '+4':
			...
			this.sendToRoom(`|html|${Dew.nameColor(this.players[next].name, true, true)} has been forced to draw 4 cards`);
			this.onDrawCard({userid: next}, 4);
			this.isPlusFour = true;
			this.timer = setTimeout(() => {
				this.sendToRoom(`|html|${Dew.nameColor(this.players[this.currentPlayer].name, true, true)} has been automatically disqualified.`);
				this.eliminate(this.currentPlayer);
			}, this.maxTime * 1000);
			break;
		case 'Wild':
			...
			this.state = 'color';
			this.timer = setTimeout(() => {
				this.sendToRoom(`${Dew.nameColor(this.players[this.currentPlayer].name, true, true)} has been automatically disqualified.`);
				this.eliminate(this.currentPlayer);
			}, this.maxTime * 1000);
			break;
		}
	...
	onUno(user, unoId) {
		...
		this.sendToRoom(Chat.html`|raw|<strong>UNO!</strong> $${Dew.nameColor(user.name, true, true)} is down to their last card!`);
	...
	onWin(player) {
		this.sendToRoom(`|raw|<div class="broadcast-green">Congratulations to ${Dew.nameColor(player.name, true, true)} for winning the game of UNO!</div>`, true);
		...
		for (let i = 0; i < this.players.length; i++) {
			Dew.addExp(Users(this.players[i]).userid, this.room, 20);
		}
...
exports.commands = {
	uno: {
		...
		disqualify: function (target, room, user) {
			...
			room.add(`|html|${Dew.nameColor(target, true, true)} has been disqualified from the UNO game.`).update();
		},
		...
		pass: function (target, room, user) {
			...
			room.game.sendToRoom(`|html|${Dew.nameColor(user.name, true, true)} has passed.`);
			room.game.nextTurn();
		},
