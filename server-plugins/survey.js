...
class Survey {
	...
	generateResults(ended, number) {
		...
		for (let i in this.surveyArray[number].repliers) {
			if (this.surveyArray[number].repliers[i]) output += `<div>${Server.nameColor(i, true)}: <i>"${Chat.formatText(this.surveyArray[number].repliers[i]) && Server.parseEmoticons(this.surveyArray[number].repliers[i])}"</i><div><br/>`;
		}
