...
exports.commands = {
	...
	slam: function (target, room, User) {
		...
		if (!this.can('lock', null, room)) return this.errorReply('Boi get slammed!');
		let targetUser = Users.get(target);
	...
	chal: function (target, room, User) {
		...
		if (!this.can('lock', null, room)) return this.errorReply('Pffft your challenge meant nothing!');
		let targetUser = Users.get(target);
	...
	poke: function (target, room, User) {
		...
		if (!this.can('lock', null, room)) return this.errorReply('No you get poked! You cannot master the capability of the epicness that is using the POKE.');
		let targetUser = Users.get(target);
	...
	sweep: function (target, room, User) {
		...
		if (!this.can('lock', null, room)) return this.errorReply('Why you always lying!?!?!');
		let targetUser = Users.get(target);
	...
	rko: function (target, room, User) {
		...
		if (!this.can('lock', null, room)) return this.errorReply('Hey, you, you aren\'t tough enough to express the usage of this!');
		let targetUser = Users.get(target);
	...
	analyze: function (target, room, User) {
		...
		if (!this.can('lock', null, room)) return this.errorReply('You lack the psychological powers to use this.');
		let targetUser = Users.get(target);
	...
	memed: function (target, room, User) {
		...
		this.parse('/lock NIIIIICE MEEEEME');
	},
	lockhammer: function (target, room, User) {
		if (!target) return this.sendReply('/lockhammer needs a target.');
		if (!this.can('lock', null, room)) return this.errorReply('Access Denied');
		let targetUser = Users.get(target);
		if (targetUser.can('root')) return this.sendReply('You cannot lock an Admin - nice try. Chump.');
	...
	dunked: function (target, room, user) {
		...
		if (!this.can('lock', null, room)) return this.errorReply('GET DUNKED ON!(access denied)');
		let targetUser = Users.get(target);
	...
	dank: function (target, room, user) {
		...
		if (!this.can('lock', null, room)) return this.errorReply('YOU ARENT DANK ENOUGH! (Access Denied!)');
		let targetUser = Users.get(target);
	...
	badtime: function (target, room, user) {
		...
		if (!this.can('lock', null, room)) return this.errorReply('kids like you should be burning in hell');
		let targetUser = Users.get(target);
