...
class MafiaTracker extends Rooms.RoomGame {
	...
	night(early = false) {
		...
		if (!early && hasPlurality) this.sendRoom(`Plurality is on ${this.players[hasPlurality] ? Server.nameColor(this.players[hasPlurality].name, true) : 'No Lynch'}`);
		this.updatePlayers();
	}
...
const pages = {
	mafia: function (query, user) {
		...
		buf += `<br/><br/><h1 style="text-align:center;">${game.title}</h1><h3>Host: ${Server.nameColor(game.host, true)}</h3>`;
		buf += `<p style="font-weight:bold;">Players (${game.playerCount}): ${Object.keys(game.players).sort().map(p => { return Server.nameColor(room.game.players[p].safeName, true); }).join(', ')}</p><hr/>`;
		...
		if (isPlayer) {
			...
			if (role) {
				buf += `<h3>${Server.nameColor(game.players[user.userid].safeName, true)}, you are a ${game.players[user.userid].getRole()}</h3>`;
		...
		if (game.phase === "day") {
			...
			for (const key of Object.keys(game.players).concat((game.enableNL ? ['nolynch'] : []))) {
				if (game.lynches[key]) {
					buf += `<p style="font-weight:bold">${game.lynches[key].count}${plur === key ? '*' : ''} ${game.players[key] ? Server.nameColor(game.players[key].safeName, true) : 'No Lynch'} (${game.lynches[key].lynchers.map(a => { return game.players[a] ? Server.nameColor(game.players[a].safeName, true) : Server.nameColor(a, true); }).join(', ')}) `;
				} else {
					buf += `<p style="font-weight:bold">0 ${game.players[key] ? Server.nameColor(game.players[key].safeName. true) : 'No Lynch'} `;
				}
				...
				if (isPlayer || isSpirit) {
					if (isPlayer && game.players[user.userid].lynching === key || isSpirit && game.dead[user.userid].lynching === key) {
						buf += `<button class="button" name="send" value="/mafia unlynch ${room.id}">Unlynch ${game.players[key] ? Server.nameColor(game.players[key].safeName, true) : 'No Lynch'}</button>`;
					} else if ((game.selfEnabled && !isSpirit) || user.userid !== key) {
						buf += `<button class="button" name="send" value="/mafia lynch ${room.id}, ${key}">Lynch ${game.players[key] ? Server.nameColor(game.players[key].safeName, true) : 'No Lynch'}</button>`;
					}
				}
				buf += `</p>`;
			}
		} else if (game.phase === "night" && isPlayer) {
			buf += `<p style="font-weight:bold;">PM the host (${Server.nameColor(game.host, true)}) the action you want to use tonight, and who you want to use it on. Or PM the host "idle".</p>`;
		}
		if (isHost) {
	...
	mafialadder: function (query, user) {
		...
		for (const key of keys) {
			buf += `<tr><td>${Server.nameColor(key, true)}</td><td>${logs[section][month][key]}</td></tr>`;
		}
		return buf + `</table></div>`;
	},
};
/** @typedef {(this: CommandContext, target: string, room: ChatRoom | GameRoom, user: Use r, connection: Connection, cmd: string, message: string) => (void)} ChatHandler */
...
const commands = {
	mafia: {
		...
		subhost: function (target, room, user, connection, cmd) {
			...
			if (oldHost) oldHost.send(`>view-mafia-${room.id}0\n|deinit`);
			if (game.subs.includes(targetUser.userid)) game.subs.splice(game.subs.indexOf(targetUser.userid), 1);
			const queueIndex = hostQueue.indexOf(targetUser.userid);
			if (queueIndex > -1) hostQueue.splice(queueIndex, 1);
