	...
	buildScreen(screen, options = {}) {
		...
		case "search":
			...
			for (let id in PINGPONG_GAMES) {
				...
				html += `<tr><td>${Server.nameColor(game.host.name, true, true)}</td><td${game.players.length >= game.options.cap ? ` style="color: red"` : ``}>${game.players.length}/${game.options.cap}</td><td>${game.options.pointsToWin}</td><td>${game.options.pin ? `<i class="fa fa-lock"></i>` : `<i class="fa fa-unlock"></i>`}</td><td><button class="button${game.players.length >= game.options.cap ? ` disabled"` : `" name="send" value="/pingpong join ${id}"`}>Join Game</button></td></tr>`;
			}
			html += `</tbody></table>`;
...
class PingPongGame {
		...
		if (!["pre-signups", "signups"].includes(this.state) && this.options.pvp && user.console.team) {
			...
			if (this.hasBall === user.userid) {
				if (user.console.team === "alpha") {
					...
					this.updateAll("play", {text: `${Server.nameColor(user.name, true, true)} has left the game of Ping Pong whilst having the ball, so their teammate ${Server.nameColor(randomTeammate, true, true)} was given the ball!`});
	...
	serve(user) {
		...
		if (this.teams["alpha"].players.includes(user.userid)) {
			...
			this.updateAll("play", {text: `${Server.nameColor(user.name, true, true)} has served the ball towards ${Server.nameColor(randomOpponent, true, true)}!`});
	...
	hit(user) {
		...
		if (hitChance) {
			...
			if (this.teams["alpha"].players.includes(user.userid)) {
				...
				let randomOpponent = otherTeam[Math.floor(Math.random() * otherTeam.length)];
				this.hasBall = randomOpponent;
				this.updateAll("play", {text: `${Server.nameColor(user.name, true, true)} has hit the ball towards ${Server.nameColor(randomOpponent, true, true)}!`});
				this.setTimer(randomOpponent);
			} else {
				let otherTeam = this.teams["alpha"].players;
				let randomOpponent = otherTeam[Math.floor(Math.random() * otherTeam.length)];
			...
			if (user.console.team === "alpha") {
				...
				this.hasBall = randomOpponent;
				this.updateAll("play", {text: `${Server.nameColor(user.name, true, true)} has missed while trying to hit the ball! Team Bravo gained a point, and ${Server.nameColor(randomOpponent, true, true)} now has the ball to serve!`});
				this.setTimer(randomOpponent);
			} else {
				this.score("alpha");
				let randomOpponent = this.teams["alpha"].players[Math.floor(Math.random() * this.teams["alpha"].players.length)];
				this.hasBall = randomOpponent;
				this.updateAll("play", {text: `${Server.nameColor(user.name, true, true)} has missed while trying to hit the ball! Team Alpha gained a point, and ${Server.nameColor(randomOpponent, true, true)} now has the ball to serve!`});
				this.setTimer(randomOpponent);
			}
		}
	}
	...
	setTimer(player) {
		...
		this.timer = setTimeout(() => {
			if (this.teams["alpha"].players.includes(Users(player).userid)) {
				...
				this.state = "awaitingServe";
				this.updateAll("play", {text: `${Server.nameColor(player, true, true)}, from Team Alpha, has failed to ${missingAction !== "awaitingHit" ? `serve` : `hit`} the ball, so the ball was given to ${Server.nameColor(randomOpponent, true, true)}, from Team Bravo!`});
				...
				let missingAction = this.state;
				this.state = "awaitingServe";
				this.updateAll("play", {text: `${Server.nameColor(player, true, true)}, from Team Bravo, has failed to ${missingAction !== "awaitingHit" ? `serve` : `hit`} the ball, so the ball was given to ${Server.nameColor(randomOpponent, true, true)}, from Team Alpha!`});
				if (randomOpponent === "COM" && !this.options.pvp) return this.comAction();
			}
		}, this.timeToHit);
	}
	...
	score(team) {
		...
		if (team === "alpha" && this.teams[team].points === this.options.pointsToWin) return this.endGame(false, true);
		if (team === "bravo" && this.teams[team].points === this.options.pointsToWin) return this.endGame(false, false);
	}
	...
	pingpong: {
		...
		options: function (target, room, user) {
			...
			case "pointsToWin":
				...
				break;
			case "timer":
				let seconds = parseInt(target.shift());
				if (isNaN(seconds) || seconds < 5 || seconds > 15) return false;
				game.options.timer = seconds;
				user.console.update(...user.console.buildScreen("setup"));
				break;
			case "pvp":
