...
exports.commands = {
	...
	badges: {
		give: function (target, room, user) {
			...
			if (Users.get(targetUser)) Users.get(userid).popup(`|modal||html|You have received a badge from ${Server.nameColor(user.name, true)}: <img src="${Db("badgeData").get(selectedBadge)[1]}" width="16" height="16"> (${selectedBadge})`);
			this.addModAction(`${user.name} gave the "${selectedBadge}" badge to ${parts[1]}.`);
			this.sendReply(`The "${selectedBadge}" badge was given to "${parts[1]}".`);
		},
		...
		take: function (target, room, user) {
			...
			userBadges = userBadges.filter(b => b !== selectedBadge);
			Db("userBadges").set(userid, userBadges);
			this.addModAction(`${user.name} took the badge "${selectedBadge}" badge from ${parts[1]}.`);
			this.sendReply(`The "${selectedBadge}" badge was taken from "${parts[1]}.`);
	...
	badgeshelp: [
		...
		"/badges delete [badge] - Deletes a badge. Requires Global @ or higher.",
