...
function display(result, user, slotOne, slotTwo, slotThree) {
	...
	if (!result) {
		display += 'Aww... bad luck, <b>' + Dew.nameColor(user) + '</b>. Better luck next time!</font>';
	}
	if (result) {
		display += 'Congratulations, <b>' + Dew.nameColor(user) + '</b>. You have won ' + slots[slotOne] + ' bucks!!</font>';
	}
	return display + '</div>';
}
