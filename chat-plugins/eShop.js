...
function writeShop() {
	...
	fs.writeFile('config/eShop.json', JSON.stringify(Exiled.eShop));
}
...
function shopDisplay() {
	...
	for (let i in Exiled.eShop) {
		if (!Exiled.eShop[i]) continue;
		output += '<tr><td style="border: 2px solid #070e96; width: 20%; text-align: center"><button name="send" value="/eshop buy ' + Exiled.eShop[i].id + '">' + Exiled.eShop[i].name + '</button></td><td style="border: 2px solid #070e96; width: 70%; text-align: center">' + Exiled.eShop[i].desc + '</td><td style="border: 2px solid #070e96; width: 10%; text-align: center">' + Exiled.eShop[i].price + '</td></tr>';
	}
	output += '</table></div>';
	return output;
}
...
//Usage notification
try {
	fs.accessSync('chat-plugins/eShop.js', fs.F_OK);
...
exports.commands = {
	...
	eshop: {
		add: function (target, room, user, connection, cmd, message) {
			...
			if (Exiled.eShop.closed) return this.sendReply('An error closed the shop.');
			target = target.split(',');
			if (!target[2]) return this.parse('/eshop help');
			if (Exiled.eShop[toId(target[0])]) return this.errorReply(target[0] + ' is already in the shop.');
			...
			Exiled.eShop[toId(target[0])] = new NewItem(target[0], target[1], target[2], isSSB);
			writeShop();
		...
		remove: function (target, room, user, connection, cmd, message) {
			...
			if (!Exiled.eShop[toId(target)]) return this.errorReply(target + ' is not in the shop.');
			delete Exiled.eShop[toId(target)];
			writeShop();
		...
		buy: function (target, room, user, connection, cmd, message) {
			...
			if (Exiled.eShop.closed) return this.sendReply('The shop is closed, come back later.');
			if (!Exiled.eShop[toId(target)]) return this.errorReply('Item ' + target + ' not found.');
			let item = Exiled.eShop[toId(target)];
			Economy.readMoney(user.userid, userMoney => {
				...
				if (item.isSSB && !Exiled.ssb[user.userid]) return this.sendReply('You need to run /ssb edit at least once before you can buy this.');
				if (item.isSSB) {
					//handle SSB pre-buy events
					switch (item.id) {
					case 'shiny':
						if (Exiled.ssb[user.userid].canShiny) return this.sendReply('You already own this.');
						break;
					case 'ffacustomsymbol':
						if (Exiled.ssb[user.userid].cSymbol) return this.sendReply('You already own this.');
						if (user.isStaff || user.group === '+') {
							//give free
							Exiled.ssb[user.userid].cSymbol = true;
							return this.sendReply('Because you have global auth you have been given this for free!');
						}
						break;
					case 'customitem':
						if (Exiled.ssb[user.userid].bought.cItem) return this.sendReply('You already own this.');
						break;
					case 'customability':
						if (Exiled.ssb[user.userid].bought.cAbility) return this.sendReply('You already own this.');
						break;
					case 'custommove':
						if (Exiled.ssb[user.userid].bought.cMove) return this.sendReply('You already own this.');
						break;
					default:
						//Unhandled
					}
				...
				Economy.writeMoney(user.userid, item.price * -1, () => {
					Economy.readMoney(user.userid, amount => {
						...
						case 'customsymbol':
							...
							break;
						case 'shiny':
							Exiled.ssb[user.userid].canShiny = true;
							writeSSB();
							break;
						case 'ffacustomsymbol':
							Exiled.ssb[user.userid].cSymbol = true;
							...
							break;
						case 'customitem':
							Exiled.ssb[user.userid].bought.cItem = true;
							writeSSB();
							break;
						case 'customability':
							Exiled.ssb[user.userid].bought.cAbility = true;
							writeSSB();
							break;
						case 'custommove':
							Exiled.ssb[user.userid].bought.cMove = true;
							writeSSB();
							break;
						default:
							...
							if (tok) {
								...
								Exiled.messageSeniorStaff(user.name + ' has purchased a ' + item.name + '.');
							}
						}
