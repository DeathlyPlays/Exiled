...
class PassTheBomb {
	...
	resetTimer() {
		this.timer = setTimeout(() => {
			let player = this.players.get(this.holder).name;
			this.room.add('|html|The bomb exploded and killed <span style = "' + Dew.nameColor(this.holder, true) + '">' + player + '</span>').update();
	...
	removeUser(userid, left) {
		...
		this.room.add('|html|' + Dew.nameColor(this.players.get(userid).name, true) + ' has ' + (left ? 'left' : 'been disqualified from') + ' the game.');
		this.players.delete(userid);
		this.madeMove = true;
		if (this.checkWinner()) {
			this.getWinner();
			...
			this.setBomb();
			let player = this.players.get(this.holder).name;
			this.room.add('|html|The bomb has been passed to ' + Dew.nameColor(player, true) + '!').update();
		}
	}
	checkWinner() {
	...
	getWinner() {
		...
		if (user) {
			let msg = '<div class = "infobox"><center>This game of Pass the Bomb has been forcibly ended by ' + Dew.nameColor(user.name, true) + '.</center></div>';
			if (!this.madeMove) {
