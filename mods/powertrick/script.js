...
exports.BattleScripts = {
	nextTurn: function () {
		...
		if (this.turn % 5 === 0 || this.turn === 1) {
			if (this.powertrick) this.removeVolatile(toId(this.powertrick));
			this.add("-message", this.startNewPowerTrick + " started!");
			if (this.startNewPowerTrick !== "Power Trick") {
				this.addVolatile(toId(this.startNewPowerTrick), this.p1.pokemon[0], "[move]: " + this.startNewPowerTrick);
			} else {
				this.volatile["powertrick"] = {
					id: 'powertrick',
					duration: 5,
				};
				this.add("-message", "The Pokemon's Attack/Special Attack and Defense/Special Defense swapped.");
				this.add("-hint", "Although its not visible, Power Trick would be in effect.");
			}
			this.powertrick = this.startNewPowerTrick;
		}
