...
github.on('push', function push(repo, ref, result) {
	...
	message += Dew.nameColor(result.pusher.name, true) + " ";
	...
	result.commits.forEach(function (commit) {
		...
		message += Dew.nameColor(commit.author.name, true) + ": " + Chat.escapeHTML(shortCommit);
		messages.push(message);
	});
...
github.on('pull_request', function pullRequest(repo, ref, result) {
	...
	message += Dew.nameColor(result.sender.login, true) + " ";
