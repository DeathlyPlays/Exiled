...
class Hangman extends Rooms.RoomGame {
	...
	generateWindow() {
		...
		if (this.guesses.length) {
			...
			if (result === 2) {
				output += Chat.html`<br />Winner: ${Server.nameColor(this.lastGuesser, true)}`;
				...
				output += Chat.html` <small>&ndash; ${Server.nameColor(this.lastGuesser, true)}</small>`;
			} else {
				output += Chat.html`<br />Guessed: ${this.guesses[this.guesses.length - 1]} <small>&ndash; ${Server.nameColor(this.lastGuesser)}</small>`;
			}
		}
