...
let proposals = FS("config/chat-plugins/proposals.json").readIfExistsSync();
...
function writeProposals() {
	FS("config/chat-plugins/proposals.json").writeUpdate(() => (
...
exports.commands = {
	...
	committee: {
		...
		give: function (target, room, user) {
			...
			this.sendReply(`|html|${Server.nameColor(councilMember, true)} has been successfully been added into the ${committee.name} Council.`);
			if (Users(councilMember)) Users(councilMember).popup(`|html|You have been added into the ${committee.name} Council by ${Server.nameColor(user.name, true)}.`);
		},
		...
		take: function (target, room, user) {
			...
			this.sendReply(`|html|${Server.nameColor(councilMember, true)} has been removed from the ${committee.name} Council.`);
			if (Users(councilMember)) Users(councilMember).popup(`|html|You have been removed from the ${committee.name} Council by ${Server.nameColor(user.name, true)}.`);
		},
